...
SoftServo *SoftServo::first;

#define NO_ANGLE (0xff)

SoftServo::SoftServo() : pin(0),angle(NO_ANGLE),pulse0(0),min16(34),max16(150),next(0)
{}
...
uint8_t SoftServo::attach(int pinArg)
{
    pin = pinArg;
    angle = NO_ANGLE;
    pulse0 = 0;
...
{
    for ( SoftServo **p = &first; *p != 0; p = &((*p)->next) ) {
	if ( *p == this) {
	    *p = this->next;
	    this->next = 0;
	    return;
...
void SoftServo::write(int angleArg)
{
...
uint8_t SoftServo::read()
{
    return angle;
}

uint8_t SoftServo::attached()
{
    for ( SoftServo *p = first; p != 0; p = p->next ) {
	if ( p == this) return 1;
    }
