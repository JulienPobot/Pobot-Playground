...
/* TC : added blanc */
char *couleur[4]={"Rouge","Vert","Bleu", "Blanc"};

/* TC : for beautiful printings ! */
char * etat_quille[4] = {"Couchée", "Debout", "Pile", "Pas une quille"};
char * positions_ponts[5] = {"Pas détectés", "Position 1 (contre le bord)", "Position 2", "Position 3", "Position 4" };
...
/* TC : testing of bridge detection */			
			if ( a_object[i].color == BLEU ) {
				fprintf(stderr, "Position des ponts : %s\n", positions_ponts [detectPonts(a_object[i].x1,
							                             			  a_object[i].x2,
										     			  a_object[i].y1,
										     			  a_object[i].y2,
										     			  a_object[i].surface)] );
			...
			/* TC : For debugging and testing (detectDistance) purposes only */			
			fprintf(stderr,
				/*"x1 : %d\tx2 : %d\ty1 : %d\ty2 : %d\t*/ "x bary : %u\t y bary : %u\t surface : %u\t pos : %u\t col : %s\t quille : %s\t distance: %d\n",
/*				a_object[i].x1,
				a_object[i].x2,
				a_object[i].y1,
				a_object[i].y2,
*/				a_object[i].x_bary,
				...
				detectDistance(a_object[i].y2, a_object[i].x_bary)
				);
		}
}
